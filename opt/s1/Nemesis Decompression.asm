...
NemDec:
		...
		lea	NemPCD_WriteRowToVDP(pc),a3	; write all data to the same location | changed into (pc), saved 4 cycles
...
NemDecToRAM:
		movem.l	d0-a1/a3-a5,-(sp)
		lea	NemPCD_WriteRowToRAM(pc),a3 ; advance to the next location after each write | changed into (pc), saved 4 cycles

NemDecMain:
		lea	(v_ngfx_buffer).w,a1
		move.w	(a0)+,d2	; get number of patterns
		bpl.s	loc_146A	; branch if the sign bit isn't set | taken from RunPLC, changed from bcc into bpl.
		lea	NemPCD_WriteRowToVDP_XOR-NemPCD_WriteRowToVDP(a3),a3	; otherwise the file uses XOR mode | changed into lea, saved 4 cycles

loc_146A:
		lsl.w	#3,d2	; get number of 8-pixel rows in the uncompressed data | changed from shifting 2 into d2 as 3 into d2.
		movea.w	d2,a5	; and store it in a5 because there aren't any spare data registers
		moveq	#7,d3	; 8 pixels in a pattern row | changed from 8 to 7 to compensate for the loop change.
		moveq	#0,d2
		moveq	#0,d4
		...
		moveq	#$10,d6	; set initial shift value | changed into moveq, saved 4 cycles
...
NemDec_ProcessCompressedData:
		...
		sub.b	(a1,d1.w),d6	; get the length of the code in bits and subtract from shift value so that the next code is read next time around
                ; changed into sub and changed d0 to d6 to avoid the ext and sub, saves 8 cycles.
...
NemPCD_WritePixel:
		...
		dbf	d3,Nem_PCD_WritePixel_Loop	; changed from subtracting 1 from d3 into simply looping d3 to pixel loop, saves 4 cycles but all moveqs for d3 that are 8 must be 7.
...
NemPCD_NewRow:
		...
		moveq	#7,d3	; reset nybble counter | changed from 8 to 7 to compensate for the loop change.
...
NemBCT_Loop:
		...
		bmi.s	NemBCT_ChkEnd	; sign bit being set signifies a new palette index | changed from bcc into bmi, removed cmpi, 8 cycles saved.
