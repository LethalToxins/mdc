...
+
		...
		andi.l	#$FFFFFF,d1
		bsr.w	Add_To_DMA_Queue ; changed from longword jsr to word bsr, saved 2 cycles.
		tst.b	(Kos_modules_left).w
		...
		movea.l	d0,a1
		move.w	(Kos_module_destination).w,d2
		bra.w	Process_Kos_Module_Queue_Init ; changed from longword jmp to word bra, saved 2 cycles.
+
		rts
...
Process_Kos_Queue_Main:
		...
		; we did some tricky shit here, but it works as this optimizes rom space, and we let it fall through to continue the rest of the modulation data.
		bsr.w   Kos_Decomp
; ---------------------------------------------------------------------------
